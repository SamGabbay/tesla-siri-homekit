...
TeslaAccessory.prototype = {
    ...
    setPowerState: function(powerOn) {
        ...
        tesla.get_vid({email: this.username, password: this.password}, function(vehicle) {
              ...
              if (powerOn) {
                tesla.autopark_request_forward({id:vehicle, latitude: '40.282517', longitude:'-74.029605'}, function(response) {
                    if (response.result)
                        that.log("Started autopark request.");
                    else
                        that.log("Error starting autopark request: " + response.reason);
                });
            }
            else {
                tesla.autopark_request_forward({id:vehicle, latitude: '40.282517', longitude:'-74.029605'}, function(response) {
                    if (response.result)
                        that.log("Stopped autopark request.");
                    else
                        that.log("Error stopping autopark request: " + response.reason);
                });
            }
        })
    },
