...
TeslaAccessory.prototype = {
    ...
    setPowerState: function(powerOn) {
        ...
        tesla.get_vid({email: this.username, password: this.password}, function(vehicle) {
              ...
              if (powerOn) {
                tesla.flash({id:vehicle}, function(response) {
                    if (response.result)
                        that.log("Started Flashing Lights");
                    else
                        that.log("Error Flashing Lights: " + response.reason);
                });
            }
            else {
                tesla.flash({id:vehicle}, function(response) {
                    if (response.result)
                        that.log("Stopped Flashing Lights.");
                    else
                        that.log("Error Flashing Lights: " + response.reason);
                });
            }
