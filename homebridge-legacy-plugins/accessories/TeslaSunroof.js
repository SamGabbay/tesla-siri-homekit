    ...
    setPowerState: function(powerOn) {
        ...
        tesla.get_vid({email: this.username, password: this.password}, function(vehicle) {
            ...
            */
            ...
            if (powerOn) {
                tesla.open_sunroof({id:vehicle, roof: 0}, function(response) {
                    if (response.result)
                        that.log("Starting to Close Sunroof");

                    else
                        that.log("Error Closing Sunroof: " + response.reason);
                });
            }
            else {
                tesla.open_sunroof({id:vehicle, roof: 3}, function(response) {
                    if (response.result)
                        that.log("Starting to Open Sunroof.");
                    else
                        that.log("Error Opening Sunroof: " + response.reason);
                });
            }
        })
    },
