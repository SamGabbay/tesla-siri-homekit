    ...
    setPowerState: function(powerOn) {
        ...
        tesla.get_vid({email: this.username, password: this.password}, function(vehicle) {
            ...
            */
            ...
            if (powerOn) {
                // tesla.open_charge_port({id:vehicle, lock: 1}, function(response) {
                //     if (response.result)
                //         that.log("Starting to Open Charge Port.");
                //     else
                //         that.log("Error Opening Charge Port: " + response.reason);
                // });
            }
            else {
                tesla.open_charge_port({id:vehicle, lock: 0}, function(response) {
                    if (response.result)
                        that.log("Unlock to Open Charge Port.");
                    else
                        that.log("Error Opening Charge Port: " + response.reason);
                });
            }
